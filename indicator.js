...
var AptUpdateIndicator = GObject.registerClass(class AptUpdateIndicator extends PanelMenu.Button {
    _init(updateManager) {
        ...
        this.updateIcon = new St.Icon({icon_name: 'security-medium-symbolic', style_class: 'system-status-icon'});
    ...
    _assembleMenu() {
        ...
        this.removeResidualMenuItem = new PopupMenu.PopupMenuItem( _('Remove') );
        this.removeResidualMenuItem.visible = false;
        ...
        this.removeAutoMenuItem = new PopupMenu.PopupMenuItem( _('Clear') );
        this.removeAutoMenuItem.visible = false;
        ...
        this.menu.addMenuItem(this.removeResidualMenuItem);
        this.menu.addMenuItem(new PopupMenu.PopupSeparatorMenuItem());
        this.menu.addMenuItem(this.autoremovablePackagesExpander);
        this.menu.addMenuItem(this.removeAutoMenuItem);
        this.menu.addMenuItem(new PopupMenu.PopupSeparatorMenuItem());
    ...
    updateStatus(updatesCount) {
        ...
        if (updatesCount > 0) {
            ...
            if (updatesCount == STATUS.UNKNOWN) {
                ...
                this.updateIcon.set_icon_name('security-medium-symbolic');
    ...
    updatePackagesStatus(index) {
        switch (index) {
            ...
            case SCRIPT.RESIDUAL:
                ...
                break;
                case SCRIPT.REMOVERESIDUAL:
                    Mainloop.idle_add(
                        this._removeResidualPackages.bind(this)
                    );
                    break;
            case SCRIPT.AUTOREMOVABLE:
                ...
                break;
            case SCRIPT.REMOVEAUTO:
                    Mainloop.idle_add(
                        this._removeAutoremovablePackages.bind(this)
                    );
                    break;
        }
    }
    ...
    _removeResidualPackages() {
        var RemoveResidualPath = Me.dir.get_child('scripts/remove-residual.sh').get_path();
        this.residualPackagesExpander.menu.removeAll();
        if (this._residualPackagesList.length == 0) {
            this.residualPackagesExpander.visible = false;
            this.removeResidualMenuItem.visible = false;
            ...
            this.residualPackagesExpander.visible = true;
            this.removeResidualMenuItem.visible = true;
            this.removeResidualMenuItem.connect('activate', () => {
                Util.trySpawnCommandLine(`x-terminal-emulator -e ${RemoveResidualPath}`);
            });
        }
    }
    ...
    _removeAutoremovablePackages() {
        var RemoveAutoPath = Me.dir.get_child('scripts/removeauto.sh').get_path();
        this.autoremovablePackagesExpander.menu.removeAll();
        if (this._autoremovablePackagesList.length == 0) {
            this.autoremovablePackagesExpander.visible = false;
            this.removeAutoMenuItem.visible = false;
            ...
            this.autoremovablePackagesExpander.visible = true;
            this.autoremovablePackagesExpander.visible = true;
            this.removeAutoMenuItem.visible = true;
            this.removeAutoMenuItem.connect('activate', () => {
                Util.trySpawnCommandLine(`x-terminal-emulator -e ${RemoveAutoPath}`);
            });
        }
    }
    ...
    _showNotification(title, message) {
        if (this._notifSource == null) {
            ...
            this._notifSource.createIcon = function() {
                return new St.Icon({ icon_name: 'security-medium-symbolic' });
            };
